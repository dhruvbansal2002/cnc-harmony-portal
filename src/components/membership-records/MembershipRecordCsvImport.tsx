...
import {
  evaluateMembershipRecordCsvPreview,
  parseMembershipRecordCsvForPreview,
  type MembershipRecordCsvImportResult,
  type MembershipRecordCsvLookups,
  type MembershipRecordCsvParseResult,
  type MembershipRecordCsvPreviewResult,
} from '../../lib/membershipRecordCsvImport'
...
function statusTone(state: MembershipRecordCsvPreviewResult['rows'][number]['importState']) {
  if (state === 'valid') {
...
interface MembershipRecordCsvImportProps {
  customers: CustomerRecord[]
  employees: EmployeeRecord[]
  membershipPlans: MembershipPlanRecord[]
  existingCompositeKeys: string[]
  onConfirmImport: (
    rows: MembershipRecordCsvPreviewResult['rows'],
  ) => Promise<MembershipRecordCsvImportResult>
  onClose: () => void
}

export function MembershipRecordCsvImport({
  customers,
  employees,
  membershipPlans,
  ...
  onConfirmImport,
  onClose,
}: MembershipRecordCsvImportProps) {
  const [parseError, setParseError] = useState<string | null>(null)
  const [parsedCsv, setParsedCsv] = useState<MembershipRecordCsvParseResult | null>(null)
  const [fileName, setFileName] = useState<string | null>(null)
  const [isImporting, setIsImporting] = useState(false)
  const [importResult, setImportResult] = useState<MembershipRecordCsvImportResult | null>(null)
  const [hasSubmitted, setHasSubmitted] = useState(false)

  const lookupData = useMemo<MembershipRecordCsvLookups>(
    () => ({
      customers,
      employees,
      membershipPlans,
    }),
    [customers, employees, membershipPlans],
  )
  ...
  const handleFileChange = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const parsed = parseMembershipRecordCsvForPreview(text, {
        existingCompositeKeys,
        lookups: lookupData,
      })
  ...
  const activePreview = useMemo(() => {
    ...
    return evaluateMembershipRecordCsvPreview(parsedCsv, {
      existingCompositeKeys,
      lookups: lookupData,
    })
  }, [existingCompositeKeys, lookupData, parsedCsv])
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      setParseError(error instanceof Error ? error.message : 'Unable to import membership records.')
    } finally {
    ...
    <section className="rounded-[2rem] border border-white/10 bg-white/5 p-6 shadow-2xl shadow-cyan-950/20 backdrop-blur sm:p-8">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.32em] text-cyan-200/80">
            Membership record CSV import
          </p>
          <h2 className="mt-3 text-2xl font-semibold tracking-tight text-white">
            Import membership records from CSV
          </h2>
      ...
      <div className="mt-6 flex flex-wrap items-center gap-4">
        ...
        <span className="rounded-2xl border border-white/10 bg-slate-950/70 px-4 py-3 text-sm text-slate-300">
          Plan/customer/employee lookups resolve on import
        </span>
      </div>
      ...
      {activePreview ? (
        <>
          <div className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
            ...
            <StatPill
              label="Existing composite keys"
              value={activePreview.existingCompositeKeys.length}
              tone="warning"
            />
          </div>

          {activePreview.duplicateCompositeKeysInCsv.length > 0 ? (
            <div className="mt-4 rounded-2xl border border-amber-500/30 bg-amber-500/10 px-4 py-3 text-sm text-amber-100">
              Duplicate records in CSV:{' '}
              <span className="font-semibold">
                {activePreview.duplicateCompositeKeysInCsv.join(' · ')}
              </span>
            </div>
          ) : null}
            ...
            <table className="min-w-[1400px] w-full border-separate border-spacing-0">
              <thead>
                <tr className="text-left text-[0.7rem] uppercase tracking-[0.28em] text-slate-500">
                  ...
                  <th className="border-b border-white/10 px-4 py-3">Customer Citizen ID</th>
                  <th className="border-b border-white/10 px-4 py-3">Customer Lookup</th>
                  ...
                  <th className="border-b border-white/10 px-4 py-3">Complimentary Items</th>
              ...
              <tbody>
                {activePreview.rows.map((row) => (
                  <tr key={`${row.lineNumber}-${row.customer_character_name}`} className="align-top">
                    ...
                    <td className="border-b border-white/5 px-4 py-4 text-sm font-semibold text-white">
                      {formatCell(row.customer_character_name)}
                    </td>
                    <td className="border-b border-white/5 px-4 py-4 text-sm text-slate-200">
                      {formatCell(row.customer_citizen_id)}
                    </td>
                    <td className="border-b border-white/5 px-4 py-4 text-sm text-slate-200">
                      {row.customer_lookup_citizen_id ?? '-'}
                      {row.customer_id ? (
                        <div className="mt-2 text-xs text-emerald-200">Linked customer found</div>
                      ) : row.customer_lookup_citizen_id ? (
                        <div className="mt-2 text-xs text-amber-200">No customer match</div>
                      ) : (
                        <div className="mt-2 text-xs text-slate-500">Optional</div>
                      )}
                    </td>
                    <td className="border-b border-white/5 px-4 py-4 text-sm text-slate-200">
                      {formatCell(row.membership_plan)}
                    </td>
                    <td className="border-b border-white/5 px-4 py-4 text-sm text-slate-200">
                      {row.issued_by_employee_citizen_id ?? '-'}
                      {row.issued_by_employee_id ? (
                    ...
                    <td className="border-b border-white/5 px-4 py-4 text-sm text-slate-200">
                      {formatCell(row.given_date)}
                    </td>
                    <td className="border-b border-white/5 px-4 py-4 text-sm text-slate-200">
                      {formatCell(row.expiry_date)}
                    </td>
                    <td className="border-b border-white/5 px-4 py-4 text-sm text-slate-200">
                      {row.complimentary_items_given ? 'Yes' : 'No'}
                    </td>
