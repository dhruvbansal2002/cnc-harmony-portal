...
import {
  createEmptyMembershipPlanFormValues,
...
export function MembershipPlanForm({
  title,
  description,
  initialValues = createEmptyMembershipPlanFormValues(),
  submitLabel,
  variant = 'panel',
  isSubmitting = false,
  error = null,
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2">
        <label className="block">
          ...
          <input
            ...
            onChange={(event) =>
              setValues((current) => ({ ...current, plan_name: event.target.value }))
            }
        ...
        <label className="block">
          ...
          <input
            ...
            inputMode="decimal"
            onChange={(event) =>
              setValues((current) => ({ ...current, plan_price: event.target.value }))
            }
            step="0.01"
            type="number"
          ...
          <label className="block">
            ...
            <select
              ...
              onChange={(event) =>
                setValues((current) => ({
                  ...current,
                  status: event.target.value as MembershipPlanFormValues['status'],
                }))
              }
              value={values.status}
            >
