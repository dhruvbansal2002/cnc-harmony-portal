...
import {
  createEmptyPriceItemFormValues,
...
interface PriceItemFormProps {
  title: string
  description: string
  initialValues?: PriceItemFormValues
  submitLabel: string
  variant?: 'panel' | 'inline'
  isSubmitting?: boolean
  error?: string | null
  onSubmit: (values: PriceItemFormValues) => Promise<void> | void
  onCancel: () => void
}
...
export function PriceItemForm({
  title,
  description,
  initialValues = createEmptyPriceItemFormValues(),
  submitLabel,
  variant = 'panel',
  isSubmitting = false,
  error = null,
  ...
  const [values, setValues] = useState<PriceItemFormValues>(initialValues)
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2">
        ...
        <label className="block">
          ...
          <input
            className="mt-2 w-full rounded-2xl border border-white/10 bg-slate-950/70 px-4 py-3 text-sm text-slate-100 outline-none transition placeholder:text-slate-600 focus:border-cyan-400/50"
            onChange={(event) =>
              setValues((current) => ({ ...current, item_name: event.target.value }))
            }
            placeholder="Optional"
        ...
        <SectionCard title="Pricing">
          <label className="block">
            ...
            <input
              ...
              inputMode="decimal"
              onChange={(event) =>
                setValues((current) => ({ ...current, item_cost: event.target.value }))
              }
              step="0.01"
              type="number"
          ...
          <label className="block">
            ...
            <input
              ...
              onChange={(event) =>
                setValues((current) => ({
                  ...current,
                  common_selling_price: event.target.value,
                }))
              }
              step="0.01"
              type="number"
              value={values.common_selling_price}
            />
          </label>
          ...
          <label className="block">
            ...
            <input
              ...
              onChange={(event) =>
                setValues((current) => ({
                  ...current,
                  government_selling_price: event.target.value,
                }))
              }
              step="0.01"
              type="number"
              value={values.government_selling_price}
            />
          </label>
        </SectionCard>
      </div>
        ...
        <label className="block">
          ...
          <select
            ...
            onChange={(event) =>
              setValues((current) => ({
                ...current,
                status: event.target.value as PriceItemFormValues['status'],
              }))
            }
            value={values.status}
          >
